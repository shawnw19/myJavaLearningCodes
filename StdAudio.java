...
public final class StdAudio {
    ...
    //new added, for simplicity the two-dimensional array is saved as a temp file
    // and played
    public static void playStereo(double[][] samples){
        String tempFile = "/tmp/"+ RandomStringUtils.randomAlphanumeric(5)+".wav";
        saveStereo(tempFile,samples);
        play(tempFile);
    }
    ...
    //new added
    public static double[][] readStereo(String filename) {
        ...
        // TODO: handle big endian (or other formats)
        else throw new IllegalStateException("audio format is not stereo");
    }
    ...
    //new added
    public static void saveStereo(String filename, double[][] samples) {
        if (filename == null) {
            throw new IllegalArgumentException("filenameis null");
        }
        if (samples == null) {
        ...
        for (int i = 0; i <length ; i++) {
            int temp1 = (short) (samples[0][i] * MAX_16_BIT);
            int temp2 = (short) (samples[1][i] * MAX_16_BIT);
            if (samples[0][i] == 1.0) temp1 = Short.MAX_VALUE;   // special case since 32768 not a short
            if (samples[1][i] == 1.0) temp2 = Short.MAX_VALUE;   // special case since 32768 not a short
            data[4*i + 0] = (byte) temp1;
            data[4*i + 1] = (byte) (temp1 >> 8);   // little endian
            data[4*i + 2] = (byte) temp2;
            data[4*i + 3] = (byte) (temp2 >> 8);   // little endian

            //System.out.println(data[4*i + 0]+" "+data[4*i + 2]);//-
        }

        // now save the file
        try {
            ByteArrayInputStream bais = new ByteArrayInputStream(data);
            AudioInputStream ais = new AudioInputStream(bais, format, length);
            if (filename.endsWith(".wav") || filename.endsWith(".WAV")) {
                AudioSystem.write(ais, AudioFileFormat.Type.WAVE, new File(filename));
            }
            else if (filename.endsWith(".au") || filename.endsWith(".AU")) {
